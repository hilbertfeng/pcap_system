...
from .forms import Upload, ProtoFilter, UserFilter
        ...
        if upload.validate_on_submit():
            ...
            if allowed_file(pcapname):
                ...
                try:
                    ...
                    print("upload exception flag me:",e)
...
def user():
    conn = pymysql.connect(host='localhost', user='root',
                           password='root', db='pcap_analysis')
    cursor = conn.cursor()
    sql = ''''''

    filter = UserFilter()
    print(filter.user_name)
    name = filter.user_name.data
    mobile = filter.mobile_no.data
    staff_no = filter.staff_no.data

    print("user find name:", name)
    print("user find mobile:", mobile)
    print("user find stuff_no :", staff_no)
    if name:
        sql = "select id, name, mobile from user where name='%s'"%name
    elif mobile:
        sql = "select id, name, mobile from user where mobile='%s'" % mobile
    elif staff_no:
        sql = "select id, name, mobile from user where id='%s'" % staff_no
    else:
    ...
    conn.close()
    if result:
        return render_template('./user/user_index.html', user_info=rlist)
